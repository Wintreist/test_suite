...
def run_suite_endpoint(request: SuiteRequest, background_tasks: BackgroundTasks):
    ...
    if current_running_test is not None:
        ...
        if running_test and running_test.status == TestStatus.RUNNING:
            raise HTTPException(status_code=409, detail=f"В данный момент уже запущен тест с ID: {current_running_test}")

    # Создаем новый тест
    test_id = str(uuid.uuid4())
    test_run = TestRunInfo(id=test_id, status=TestStatus.PENDING, created_at=datetime.now())
    ...
    return {"test_id": test_id, "status": "accepted", "message": "Тест принят к исполнению"}
